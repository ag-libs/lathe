...
-- `lathe/testOutput` handler. This adapter only routes into it and, once a run finishes,
-- decorates its stack frames (below). `next_token` and `open_output` are re-exposed so build_spec
-- and any user keymap bound to open_output keep their existing call sites.
...
--- onto the buffer that output.reset() already emptied.
