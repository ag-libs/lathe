...
- Source staleness after a change or branch switch is detected via compile stamps and surfaced as a
  Neovim sync prompt (WS-1, resolved as detect → prompt); types created or renamed in closed files are
  discovered after the prompted sync.
