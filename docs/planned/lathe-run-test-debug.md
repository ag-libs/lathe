...
| `.lathe/run.json` | `lathe:sync` (plugin, create-if-absent) + user edits | server | Named run/debug configs; thin overlay only, gitignored (§8) |
...
Lathe stores named run/debug configurations in a single gitignored JSON file, `.lathe/run.json`,
at the workspace root — one file for the whole reactor, alongside `workspace.json`.
It is generated by `lathe:sync` (create-if-absent) and read by the server; the client never constructs
Java command lines.

Being gitignored is deliberate:
configs hold per-developer absolute paths, local class-/module-path additions, and `env` entries that
may carry secrets, so they are machine-local and never committed.
The trade-off is recorded in §9: a `rm -rf .lathe` reset regenerates the `default` stubs but permanently
loses hand-authored named configs, since they are neither in git nor regenerable.

### 8.1 File shape

The file is a flat JSON array of run items — no wrapper object, no schema version.
Every item has the same shape; the only variation is which fields are non-null:

```json
[
  { "moduleRel": "app", "name": "default", "kind": "main",
    "mainClass": null, "selector": null,
    "args": null, "jvmArgs": null, "env": null, "cwd": null,
    "classpathAppend": null, "modulePathAppend": null, "debug": null },

  { "moduleRel": "app", "name": "default", "kind": "test",
    "mainClass": null, "selector": null,
    "args": null, "jvmArgs": null, "env": null, "cwd": null,
    "classpathAppend": null, "modulePathAppend": null, "debug": null },

  { "moduleRel": "app", "name": "prod", "kind": "main",
    "mainClass": "com.example.app.Main", "selector": null,
    "args": null, "jvmArgs": ["-Dprofile=prod"], "env": { "APP_ENV": "prod" }, "cwd": null,
    "classpathAppend": ["config/prod"], "modulePathAppend": ["libs/plugin.jar"], "debug": null }
]
```
...
| Field | Type | Meaning |
|---|---|---|
| `moduleRel` | string | Module the item targets (the `<rel>` key of §2.1) |
| `name` | string | Config name; `"default"` is reserved (§8.2) |
| `kind` | `"main"` \| `"test"` | Run kind; the extension point for future kinds |
| `mainClass` | string \| null | Pinned main target (`kind:main` only) |
| `selector` | object \| null | Pinned test target (`kind:test` only) |
| `args` | array \| null | Program arguments |
| `jvmArgs` | array \| null | Additional JVM arguments |
| `env` | object \| null | Environment variables |
| `cwd` | string \| null | Working directory |
| `classpathAppend` | array \| null | Extra class-path entries |
| `modulePathAppend` | array \| null | Extra module-path entries |
| `debug` | bool \| null | Launch under the debugger (reserved; behavior deferred to §12.9) |

The uniqueness key is `(moduleRel, kind, name)`.
Every overlay field is nullable and `null` means "use the generated default"; an all-null item is
identical in effect to no item at all.
In code this is a single `RunItem` record deserialized identically for every array element, with a
compact constructor that validates the `kind`↔target relationship and the reserved-name rule of §8.2.

### 8.2 Defaults, named configs, and target resolution

`name: "default"` is reserved for the per-`(moduleRel, kind)` overlay applied to gutter/neotest runs.
A `default` item **may not** pin `mainClass`/`selector`:
it is inherently editor-derived, applied to whatever runnable the user launches from the gutter.
`lathe:sync` seeds two `default` items per module (`main` + `test`, all overlay fields null) when it
creates the file, so every module ships with editable stubs.

Any other `name` is a user-added named config, selected explicitly (`:LatheRun {name}`, §12.8).
...
- **Editor-derived** — target null → applies the overlay to the runnable under the cursor at launch,
...
- `null` — use the generated default; no overlay for that field.
...
```
Neovim: :LatheRun {name}            (gutter/neotest run resolves the module's `default` item)
  ├─ server reads .lathe/run.json fresh (lock-aware)
  ├─ resolve RunItem by (moduleRel, kind, name); target editor-derived when unpinned
...
- **Run configs are machine-local and disposable** — `.lathe/run.json` is gitignored, so hand-authored
  named configs are not shared with teammates and are permanently lost on a `rm -rf .lathe` reset (the
  `default` stubs regenerate on the next `lathe:sync`; custom configs do not). This is the accepted cost
  of keeping secret-bearing, per-developer launch settings out of git (§8).
- **Late-added modules are not re-seeded** — `lathe:sync` writes `.lathe/run.json` only when it is
  absent, so a module added to the reactor after the file exists gets no auto-seeded `default` stubs. It
  is harmless — a missing `default` item falls back to the pure-derived launch — and the user can add
  stubs by hand or delete the file to regenerate. A future "append missing defaults, never modify
  existing" merge is a small additive change if this ever bites.
...
| `SyncMojo` → `RunConfigSeeder` | Create `.lathe/run.json` if absent, seeding two `default` `RunItem`s per module (`main` + `test`, all overlay fields null); never touches an existing file | `seed(project)` |
...
| `run.RunItem` (record) | One `.lathe/run.json` array element / command payload overlay | validating compact ctor (kind↔target, reserved-name rule) |
| `run.RunConfigReader` | Read `.lathe/run.json` fresh, lock-aware; partition by `(moduleRel, kind, name)` | `read(): List<RunItem>` |
...
This slice is two-sided: a Maven-plugin writer that seeds the file, and a server reader/resolver that
applies it. `debug` is reserved in the schema only; its launch behavior is deferred to §12.9.

**Scope (plugin):**

- Add `RunConfigSeeder`, invoked from `SyncMojo`: create `.lathe/run.json` when absent, seeding two
  `default` `RunItem`s per module (`main` + `test`, all overlay fields null); never modify an existing
  file.

**Scope (server):**

- Add the `RunItem` record (§8.1) with a validating compact constructor: `kind`↔target relationship and
  the reserved `default`-name (target-less) rule.
- Add `RunConfigReader` — read `.lathe/run.json` fresh and lock-aware, partition by
  `(moduleRel, kind, name)`.
...
- Add `config.list` and wire `run`/`debug` to accept a `(moduleRel, kind, name)` selection plus optional
...
- Reserved `default` name may not pin a target; `(moduleRel, kind, name)` uniqueness enforced.
- Seeder is create-if-absent only — never clobbers user edits.

**Verification:**

- Unit tests for `RunItem` parsing/validation and per-field overlay semantics (append, merge, additive
  paths, null-as-default).
- Seeder test: creates stubs when absent, leaves an existing file untouched.
...
2. **`systemPropertyVariables`** — design settled (hybrid: sync keys + fork values), implementation
   **deferred**; see §15.1.
3. ~~**`.lathe/run.json` schema and skeleton ownership**~~ — resolved (§8): single gitignored
   `.lathe/run.json` at the workspace root; a flat array of uniform `RunItem`s keyed by
   `(moduleRel, kind, name)`, no wrapper/schema version; `lathe:sync` creates it (create-if-absent) and
   seeds the per-module `default` stubs; the server is reader/resolver only.
