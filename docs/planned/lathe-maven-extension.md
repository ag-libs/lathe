...
- Locate `org.apache.maven.plugins:maven-compiler-plugin` via `build.getPluginsAsMap()`.
  The spike (§9) confirmed this returns the **default lifecycle-bound** compiler plugin even when the
  POM does not declare it, so the fallback "create it if absent" branch is rarely taken.
- Add `io.github.ag-libs:lathe-compiler:${latheVersion}` to its `<dependencies>` if not already
  present. This dependency **does** reach the plugin's classrealm at execution (§9), so
  `compilerId=lathe` resolves.
- Select the Lathe compiler by setting the **project property**
  `maven.compiler.compilerId=lathe` — **not** by writing a `<compilerId>` node into the plugin
  `<configuration>`. The spike (§9) proved that a plugin-level `<compilerId>` config element added in
  `afterProjectsRead()` does **not** drive the `default-compile`/`default-testCompile` executions
  (the mojo param defaults to `${maven.compiler.compilerId}`, which the property sets and the config
  node does not).

Merge policy is **extension wins** (§5): the property is set unconditionally, so a project's stale
`lathe-compiler` version or a different `maven.compiler.compilerId` is overwritten and the effective
build always matches the extension. Unrelated compiler configuration (`<release>`,
`<annotationProcessorPaths>`, `<compilerArgs>`) is left untouched — the property/dependency changes do
not disturb the existing `<configuration>` tree.
...
- ~~**Sisu registration mechanics**~~ — **resolved (§9):** a `@Named("…")` + `@Singleton`
  `AbstractMavenLifecycleParticipant` with a `META-INF/sisu/javax.inject.Named` index listing the
  class (one FQCN per line) is discovered and invoked. No `META-INF/plexus/components.xml` is needed.
  In the real module the index is generated by the `sisu-maven-plugin` rather than hand-written.
...
## 9. Spike outcomes (validated / learned)

A throwaway `AbstractMavenLifecycleParticipant` (a scratch extension + a scratch project registering
it via `.mvn/extensions.xml`, no Lathe POM config, using the already-installed `lathe-compiler`) was
run under Maven 3.9.x. Findings:

- **Plugin-dependency injection reaches the plugin realm.** Adding `lathe-compiler` to
  `maven-compiler-plugin`'s `<dependencies>` in `afterProjectsRead()` puts it on the compiler plugin's
  classrealm at execution (`-X`: `Included: io.github.ag-libs:lathe-compiler:…`), so `compilerId=lathe`
  resolves. This was the design's biggest risk — **confirmed viable**. The shim ran end-to-end:
  `.lathe/<rel>/lsp-params-classes.json` and mirrored `.lathe/<rel>/classes/…` were written.
- **Compiler selection must use the property, not the config node.** A plugin-level
  `<compilerId>lathe</compilerId>` added in `afterProjectsRead()` left the effective param at
  `compilerId = javac` (no shim). Setting the project property `maven.compiler.compilerId=lathe`
  works. §3.4(a) reflects this.
- **`getPluginsAsMap()` returns the default lifecycle-bound compiler plugin** even when the POM does
  not declare `maven-compiler-plugin`; both the POM-declared and default-bound cases produced the shim
  output. The "create if absent" branch is a rare fallback.
- **Sisu registration** via `@Named` + `@Singleton` + a `META-INF/sisu/javax.inject.Named` index
  (one FQCN per line) is sufficient; no `plexus/components.xml`.
- **mvnd caches core extensions per daemon** — dev iteration on the extension needs `mvnd --stop`
  after reinstalling. Irrelevant to end users (register once); relevant to the extension's own
  build/test loop.
