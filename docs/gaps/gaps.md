...
## EG-052 — Go-to-definition on a generated type lands on the stale Maven `target/generated-sources` copy after an in-editor edit (needs `mvn`)

**Status: accepted — Target: next**

### Observed behaviour

After editing an annotation-processed source in the editor (e.g. adding a component to a `@Builder`
record) and saving, completion immediately reflects the regenerated API — the new setter is offered —
but `textDocument/definition` on the generated type still resolves to Maven's
`target/generated-sources/annotations/…Builder.java`, which only a `mvn` run updates. The navigated
file therefore shows the pre-edit code, and in the incremental-regen state a spurious
`…Builder is not abstract` diagnostic. Re-running `mvn` is currently required to make `gd` correct;
it should work without it.

Reproduce on the public `multi-module` invoker fixture:

```bash
mvn verify -pl lathe-maven-plugin -Dinvoker.test=multi-module   # build .lathe + generated sources
# def on `UserBuilder` in Main.java resolves to the Maven target copy, not .lathe:
python3 dev/explore.py \
  lathe-maven-plugin/target/it/multi-module/app/src/main/java/com/example/app/Main.java def <line>:<col>
# → app/target/generated-sources/annotations/com/example/app/UserBuilder.java
# Now add a component to User.java (e.g. `Instant createdAt`) and save in-session:
#   completion after `UserBuilder.builder().` offers createdAt(Instant)  ← fresh (in-memory / .lathe regen)
#   def on UserBuilder still lands on the stale target copy (no createdAt) ← the gap
```

Also live-probed on a large real Maven workspace: `def` on a `@Builder`-generated type resolves to
`…/target/generated-sources/annotations/…Builder.java`, while that module's
`.lathe/…/generated-sources` holds **0** generated `.java` files — generated sources are not mirrored
into `.lathe`.

### Root cause (suspected — confirm in the definition provider)

Generated sources are not copied into `.lathe`; they live in Maven's `target/generated-sources/annotations`,
and definition resolves there via javac's sourcepath attribution. Lathe's on-save incremental compile
regenerates into `.lathe/<module>/generated-sources` for its own analysis (so completion and symbols
are fresh), but the definition location keeps pointing at the untouched Maven `target` copy — the
symbol table and the go-to-definition source location disagree after an in-editor edit.

### Proposed direction

Make `gd` correct without `mvn`: after an in-editor regen, resolve generated-type definitions to the
freshly regenerated `.lathe/<module>/generated-sources` copy (the one Lathe keeps in sync) rather than
Maven's `target` copy — e.g. prefer `.lathe` generated-sources on the definition sourcepath, or map
the resolved location onto the regenerated file. Steady-state behaviour (both copies in sync) must be
preserved. No ad-hoc parsing — javac elements plus the existing sourcepath / `.lathe` plumbing only.

### Regression targets

A server test that builds the fixture, edits `User.java` to add a component, saves, then asserts
`textDocument/definition` on `UserBuilder` resolves to a source containing the new component (the
fresh copy) — and that completion and definition agree — all without re-running Maven.
...
`checkSourceStaleness` derives their reactor-relative `moduleRel`s and carries them to `requestSync`.
`syncScope` falls back to a **full** reactor build once **≥ `FULL_SYNC_PERCENT` (75%)** of the reactor's
modules changed (or on a POM/structural change) — otherwise it targets the changed modules; a full build
is also the only one that regenerates `workspace.json`, which `-pl` cannot. The prompt reflects the real
action: a partial refresh names ≤ `MODULE_NAME_LIMIT` (3) modules in brackets (else a count), a full
refresh shows a count, and a structural change is generic (*"Maven project changed. Lathe will run a full
refresh."*). `LatheSyncParams.modules` carries the selectors; the client builds `mvn … -pl <modules> -am
<goal>` (`-am` is required because a partial `process-test-classes` cannot resolve reactor-sibling deps
from `~/.m2`; `-amd` is intentionally omitted — dependents are WS-6). New modules are structural (they
edit a tracked parent POM → `POM_CHANGED` → full), so they are handled by the POM path, not the targeted
scan. Verified end-to-end in a multi-module workspace (a `module-info.java` touch in one module → prompt
names that module → `mvn … -pl <module> -am process-test-classes`). Manual `:LatheSync <module>` targeting
