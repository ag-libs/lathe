...
Neovim **diffview** git-diff view (`diffview:///…/.git/:0:/…/SpendLimitChecker.java`) — makes the
server throw on both the initial `didOpen` and on follow-up requests (`textDocument/foldingRange`
observed). The failure surfaces to the client as an `InternalError` RPC and, in Neovim, as an
`UnhandledPromiseRejection`:

```
RPC[Error] code_name = InternalError, message = "Internal error."
Caused by: java.nio.file.FileSystemNotFoundException: Provider "diffview" not installed
    at java.base/java.nio.file.Path.of(Path.java:210)
    at ….LatheUri.toPath(LatheUri.java:11)
    at ….WorkspaceSession.routeCompiler(WorkspaceSession.java:2121)
    at ….WorkspaceSession.foldingRangeFuture(WorkspaceSession.java:1815)  # also compileAndPublish via onOpen
```

The server log shows it first accepts the buffer — `INFO [open] diffview:///…SpendLimitChecker.java`
— then `SEVERE [worker] task failed` with the stack above.

### Root cause

`LatheUri.toPath` is `Path.of(URI.create(uri))`, which only resolves for the `file` scheme; any other
scheme (`diffview://`, and likely `fugitive://`, `gitsigns://`, `octo://`) has no NIO
`FileSystemProvider`, so `Path.of` throws `FileSystemNotFoundException`. Nothing at the LSP boundary
filters the document URI: every `LatheTextDocumentService` handler forwards the raw URI into the
session, which routes it to `routeCompiler` → `LatheUri.toPath`. Diffview opens a *real* Java file
through its own scheme, so Lathe should simply ignore the virtual buffer rather than treat it as a
source.

### Proposed fix

Drop non-`file` documents at the LSP boundary (server-side, primary — a language server must not
crash on a non-`file` URI):

1. Add a unit-tested `LatheUri.isFileDocument(String uri)` returning `true` only when the parsed
   scheme equals `file` (null/malformed → `false`).
2. Guard `LatheTextDocumentService`: notifications (`didOpen/didChange/didClose/didSave`) early-return
   for non-file URIs so the buffer is never tracked; requests short-circuit to a
   `CompletableFuture.completedFuture(<empty>)` of the handler's own type (empty list, or `null` for
   `Hover`/`SignatureHelp`) instead of routing into the session. Because `didOpen` is dropped, the
   request guards are defense-in-depth. Emit one `FINE` `[skip] non-file uri <uri>` line.

Deliberately no generic higher-order wrapper across the ~20 handlers — a per-method one-line guard is
simpler than generics-plus-empty-suppliers.

Complementary client-side hardening (separate, optional NV follow-up): have `lua/lathe.lua` avoid
attaching the Lathe client to buffers whose name scheme isn't `file` (guard the attach path on
`buftype` / `vim.uri_from_bufnr`). The server fix stands on its own; this just avoids the wasted
round-trip.

### Probe commands

Not probeable through `explore.py` (it only opens `file://` documents). Reproduced by opening a Java
file in Neovim's `:DiffviewOpen` and triggering folding (or any request) on the diff buffer.

### Regression targets

- `LatheUriTest.isFileDocument_fileScheme_returnsTrue`
- `LatheUriTest.isFileDocument_customScheme_returnsFalse` (e.g. `diffview://…`)
- `LatheUriTest.isFileDocument_malformedUri_returnsFalse`
- `LatheTextDocumentServiceTest.foldingRange_nonFileUri_returnsEmptyWithoutThrowing`
  (and a `didOpen` no-op case for the same URI)
