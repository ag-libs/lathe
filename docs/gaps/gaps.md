...
## CQ-0051 — Void-returning members dropped from member completion (value-slot filter fires at a statement)
...
Tier: correctness
Failure mode: missing-candidate
Owner component: CompletionCandidateRanker / MemberAccessCompleter / completion cache

Summary:
Member-access completion after `Receiver.` sometimes omits all `void`-returning methods, so a valid
statement call such as `Thread.sleep(...)` (or a util type's `sleepXxx(...)`) is not offered.
Retrying the same completion a moment later returns the full set.

Reproduction — deterministic (value slot, exposes the filter):
Complete after `Type.` in a value slot versus a statement, using a JDK type with void static methods
(`java.lang.Thread` has void `sleep`/`yield`):
```bash
# statement — offers all static members, including void sleep/yield
python3 dev/explore.py <file> inject "Thread." expect sleep yield
# value slot — void sleep/yield are filtered out
python3 dev/explore.py <file> inject "int x = Thread." expect sleep   # FAILS: sleep absent
```

Reproduction — editor (the actual bug: a statement is misclassified):
Type `Type.` on its own line immediately followed by a `.`-chained statement
(`local.method(...);`), then request completion right after the dot: the result omits void methods;
a second request returns them.

Root cause (two defects compound):
1. **Void filter.** `CompletionCandidateRanker.valid()` returns `!voidMethod(candidate)` whenever the
   slot is value-sensitive (`valueSensitiveContext = valueContext() || expectedValue instanceof
   ExpectedValue.Type`). A void call yields no value, so it is hidden where a value is required. In a
   genuine value slot (`int x = Type.`) this is defensible; at a statement it is wrong, because
   `Type.voidMethod();` is a valid statement.
2. **Stale completion cache drives the misclassification.** `SourceAnalysisSession.complete()` builds
   the request with `cache.get(uri)` and never checks it matches the current content/version
   (contrast `runnables()`/`semanticTokens()`, which use `attributedAnalysis(uri, content,
   expectedVersion)`). `MemberAccessCompleter` only reattributes when the cached receiver is `null`
   or an `ERROR`/`TYPEVAR` type; when a stale cache still resolves the receiver to a valid type,
   `shouldReattribute` is false and completion answers from the stale tree
   (`resolveAnalysis → req.cached().analysis()`, logged `reattributed=false`). The stale tree
   classifies the statement slot as value-sensitive, so defect (1) drops the void methods. The
   debounced recompile of the just-typed change lands afterwards and refreshes the cache, so a retry
   shows the full set.

Evidence (nvim LSP log, timestamps abbreviated):
```
.540 [change] …Test.java
.548 [completion] resolve receiver=|Type| type=…Type static=true reattributed=false
.551 [completion] proposals count=5 labels=[…the two void methods absent…]
.906 [compile:open] …Test.java            ← refreshing recompile lands 358ms AFTER the completion
```
Completion fires 8 ms after the edit and 358 ms before the refreshing compile; the two dropped labels
are exactly the type's two `void` methods.

Expected Lathe behavior:
At a statement-position member access, void-returning methods must be offered. More generally,
completion must not decide a value/keyword-sensitive filter from an analysis that predates the current
buffer; a stale cache should trigger reattribution (as other features do), and the void filter should
apply only inside a confirmed value slot.

Proposed fix (to discuss):
- Validate the cached analysis against the current content/version in `complete()` (mirror
  `attributedAnalysis`) and reattribute on mismatch, so the slot is classified from the current tree.
- And/or narrow the void filter so it never applies to a bare statement-position member access.

Regression target:
`CompletionEngineTest` / `MemberAccessCompleterTest`: (a) member access on a type with a void static
method at a statement position offers the void method; (b) the value-slot form filters it; (c) a
completion whose cached analysis predates the current content reattributes rather than returning a
value-sensitive subset.

Notes:
Distinct from CQ-0052: here the void methods belong to the already-resolved receiver type and are
dropped by ranking, not by resolution.

## CQ-0052 — Member completion on an unimported ambiguous simple name surfaces only one candidate type
...
Owner component: MemberAccessCompleter.fallback / TypeResolver / WorkspaceTypeIndex

Summary:
When `Receiver.` uses an unimported simple name that matches several classpath types — e.g. multiple
`ThreadUtils` classes (`org.apache.commons.lang3.ThreadUtils`, `org.apache.commons.io.ThreadUtils`,
`org.apache.kafka.common.utils.ThreadUtils`, plus a project's own) — member completion resolves the
name to exactly one candidate type and offers only that type's members. Members unique to the other
same-named types (e.g. commons-lang3's `sleep`/`sleepQuietly`) are never offered, even though the
missing-import code action does list all candidates.

Reproduction:
```bash
# only one ThreadUtils type's members are offered; commons-lang3 sleepQuietly never appears
python3 dev/explore.py <file> inject "ThreadUtils." expect sleepQuietly   # FAILS
```

Expected Lathe behavior:
Aggregate static-member candidates across all same-named types, each carrying its own auto-import edit
(IntelliJ behavior), so the user can pick the intended type's member.

Regression target:
`MemberAccessCompleterTest`: unimported `Type.` whose simple name matches ≥2 indexed types offers
members from each, with the correct per-candidate import edit.

Notes:
Related to CQ-0051 but independent: this is a resolution/aggregation gap; CQ-0051 is a ranking-filter
gap.
