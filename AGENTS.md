...
- **General Rules**:
    ...
    * Always add a blank line after the closing `}` of any block statement (`if`, `for`, `while`, `try`, `switch`) when another statement follows, unless it is the last statement in an enclosing block.
